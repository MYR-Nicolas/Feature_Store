...
import pandas as pd
from ELT.load import load_df_to_gcs, upload_to_gcs


def test_upload_file_to_gcs_success(mocker):
    """Test that a local file is successfully uploaded to GCS."""
...
def test_upload_uses_default_bucket(mocker):
    """Test that the default bucket is used when none is provided."""
...
def test_load_df_to_gcs(mocker, tmp_path):
    """Test the full flow from DataFrame export to GCS upload."""

    mock_upload = mocker.patch(
        "ELT.load.upload_to_gcs",
        return_value="gs://bucket/path.parquet",
    )
